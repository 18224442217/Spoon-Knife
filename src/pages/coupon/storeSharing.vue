        ...
        <van-list v-model="loading"
          ...
          <div
            ...
            <div class="shop-top">
              <div class="shop-top-left">
                ...
                <img v-if="!shoplist.logo"
                     ...
                     class="shop-logo">
                <div>
                  <p class="title">{{ shoplist.merchantName }}</p>
                  <p class="merchantAddress">{{ shoplist.merchantAddress }}
                    <!-- <van-icon name="arrow"
            ...
            <div class="shop-center">
              <div v-for="(item,index) in ymBMerchantCouponReturnVO"
                   :key="index"
                   :class="item.state===1 ? 'shop-list' :'shop-list1'">
                <span :class="item.state===1 ? '' :'color8E8E8E'"
                      class="countTotal"><span>剩{{ item.count }}张</span>  <span>分享</span></span>
            ...
            <!-- <div v-if="shoplist.ymBMerchantCouponReturnVO.length<shoplist.totalCouponNumber"
                 class="more"
  ...
  methods: {
    ...
    onLoadcon() {
      ...
      if (this.$route.query.data) {
        ...
        this.ymBMerchantCouponReturnVO = [] // 清空数组
        const params = {
          merchantId: '1394975011009843202',
          lng: this.lng,
          lat: this.lat,
        ...
        console.log(params, '=====')
        queryshopAi.queryNearMerchantById(params).then(res => {
          console.log(res)
          res.map(item => {
            item.page = {
              couponPageSize: 4,
          ...
          if (this.ymBMerchantCouponReturnVO.length >= res[0].totalMerchantNumber) {
            this.finished = true // 结束加载状态
          }
          this.page1.merchantCurrentPage++
          console.log(this.ymBMerchantCouponReturnVO)
        }).catch(err => {
          console.log(err)
          this.finished = true // 停止加载
        })
          ...
          if (this.getStatus() === BMAP_STATUS_SUCCESS) {
            ...
            myGeo.getLocation(new BMap.Point(r.point.lng, r.point.lat), data => {
              if (data.addressComponents) {
                ...
                const params = {
                  merchantId: '1394975011009843202', // 1382015274710585345 1386290822832050177 1394825079464513537
                  lng: _this.lng,
                  lat: _this.lat,
                ...
                queryshopAi.queryNearMerchantById(params).then(res => {
                  ...
                  _this.loading = false
                  _this.ymBMerchantCouponReturnVO.push(...res.ymBMerchantCouponReturnVO)
                  if (res.ymBMerchantCouponReturnVO.length === 0) { // 判断获取数据条数若等于0
                    _this.ymBMerchantCouponReturnVO = [] // 清空数组
                    _this.finished = true // 停止加载
                  }
                  if (_this.ymBMerchantCouponReturnVO.length >= res.ymBMerchantCouponReturnVO[0].totalMerchantNumber) {
                    _this.finished = true // 结束加载状态
                  }
                  _this.page.merchantCurrentPage++
                  console.log(_this.ymBMerchantCouponReturnVO)
                }).catch(err => {
                  console.log(err)
                  _this.finished = true // 停止加载
                })
