          ...
          <div v-for="(item,index) in shoplist"
              ...
              <div v-for="(item,index) in item.ymBMerchantCouponReturnVO"
                ...
                <div class="shop-hintbox"><p class="shop-hint time">过期时间: {{ item.endTime.slice(0,11) }}</p>
                  <!-- <span :class="item.state===1 ? '' :'color8E8E8E'"
                        ...
                        @click="mycouponsee(item)">查看详情</span> -->
                </div>
            ...
            <div class="shop-topbox">
              ...
              <div v-for="(item,index) in item.ymBMerchantCouponReturnVO"
                   :key="index"
                   class="shop-details">
                <div><span>使用规则</span><span>{{ item.useRule }}</span></div>
                <div><span>剩余张数</span><span>{{ item.count }}</span></div>
                <div><span>过期时间</span><span>{{ item.endTime.slice(0,10) }}</span></div>
              </div>
            </div>
          </div>
        </van-list>
...
.shop-details div span:last-child{
  margin-left: 40px;
  font-size: 24px;
}
.shop-details div span{
  color:#4C4C4C ;
}
.shop-details div{
  background: url(../../assets/img/fx1.png)no-repeat 0 center;
